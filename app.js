...
const App = (() => {
  let state = {
    ...
    stats: {},
    profile: {},
    currentPlan: null,   // plan being edited
    currentSession: null, // active workout
    sessionStartTime: null,
    timerInterval: null,
    workoutLogs: {},     // { exerciseId: { setNum: { reps, weight, completed } } }
  };
  ...
  function renderHome() {
    ...
    let greeting = 'night';
    for (const [h, g] of greetMap) { if (hour >= h) greeting = g; }
    ...
    document.getElementById('home-streak').textContent = (state.stats.streak || 0) + (state.stats.streak === 1 ? 'd' : 'd');
  ...
  function renderTodayCard() {
    ...
    const html = `
      <div class="workout-preview">
        ...
        ${plan.notes ? `<p style="font-size:13px;color:var(--white-50);margin-bottom:12px;">${escHtml(plan.notes)}</p>` : ''}
        <div class="workout-preview-exercises">
          ${plan.exercises.slice(0, 4).map(ex => `
            <div class="wp-exercise">
              ...
              <span class="wp-exercise-meta">${ex.sets}×${ex.reps}${ex.weight ? ' · ' + ex.weight + 'kg' : ''}</span>
            </div>`).join('')}
          ${exCount > 4 ? `<div style="font-size:12px;color:var(--white-50);text-align:center;">+${exCount-4} more</div>` : ''}
        </div>
  ...
  function openPlanForm(planId = null) {
    ...
    document.querySelectorAll('.day-btn').forEach(btn => {
      btn.classList.toggle('selected', state.currentPlan?.weekdays?.map(Number).includes(Number(btn.dataset.day)) || false);
    });
  ...
  function editPlan(id) { openPlanForm(id); }
  ...
  async function savePlan() {
    ...
    const payload = { name, notes: document.getElementById('plan-notes').value.trim(), weekdays, exercises };
  ...
  function updateSetLog(sessionId, exId, setNum) {
    // Debounced save — just track state locally; will save on check
  ...
  async function toggleSet(sessionId, exId, setNum, totalSets) {
    ...
    const log = state.workoutLogs[exId]?.[setNum] || {};
    ...
    await api('/api/set_logs', 'POST', {
      session_id: sessionId, exercise_id: exId, set_num: setNum,
      reps, weight, completed: isChecked ? 1 : 0
    });
  ...
  function renderChart() {
    ...
    const bw = (w - pad.l - pad.r) / history.length;
    ...
    history.forEach((d, i) => {
      ...
      grad.addColorStop(0, d.count > 0 ? '#00d4ff' : 'rgba(255,255,255,0.08)');
      grad.addColorStop(1, d.count > 0 ? 'rgba(0,87,255,0.6)' : 'rgba(255,255,255,0.04)');
      ctx.fillStyle = grad;
      ctx.beginPath();
      const r = 4;
      ctx.roundRect(x, y, bWidth, barH, [r, r, 0, 0]);
      ctx.fill();
  ...
  async function resetData() {
    ...
    state.plans = []; state.stats = {}; state.profile = {};
    renderHome(); renderPlans(); renderProfile(); renderStats();
  ...
  async function exportData() {
    ...
    a.href = url; a.download = 'liftlog-export.json'; a.click();
  ...
  window.addEventListener('beforeinstallprompt', e => {
    ...
    hr.innerHTML = `<button onclick="App.installPWA()" style="font-size:12px;color:var(--cyan);font-weight:600;background:var(--cyan-glow);padding:5px 12px;border-radius:20px;border:1px solid rgba(0,212,255,0.3)">Install App</button>`;
  });
  ...
  return {
    goTo, openPlanForm, closePlanForm, editPlan, addExerciseRow, savePlan, deletePlan,
    startWorkout, finishWorkout, toggleSet, updateSetLog,
    saveProfile, resetData, exportData, installPWA
  };
})();
